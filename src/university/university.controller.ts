...
export class UniversityController {
  ...
  @Get(':id/students')
  getUniversityWithStudentById(@Param('id', ParseObjectIdPipe) id: string) {
    return this.UniversityService.getUniversityWithStudentById(id);
  }

  @Delete('')
  deleteUniversityByAll(@Res() response: any): Observable<Response> {
    return this.UniversityService.deleteUniversityByAll().pipe(
      map((student) => {
        return response
          .status(200)
          .send({ message: 'Universities deleted success' });
      }),
    );
  }
  ...
  deleteUniversityById(
    ...
    return this.UniversityService.deleteUniversityById(id).pipe(
      map((student) => {
        return response
          .status(200)
          .send({ message: 'University deleted success' });
      }),
    );
  }
}
